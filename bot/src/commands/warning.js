...
export default {
  ...
  async execute({ message, args }) {
    ...
    if (!target) return message.reply({ embeds: [errorEmbed('Mention a valid user. Usage: `?warning @user`')] });

    const data = getGuildData(message.guild.id);
    const list = data.warnings[target.id] || [];
    if (!list.length) {
      return message.reply({ embeds: [infoEmbed(`**${target.user.tag}** has no warnings.`, '📋 Warnings')] });
    }
    const lines = list.slice(-15).map((w, i) => `**${i + 1}.** ${w.reason}\n   • by <@${w.moderatorId}> • <t:${Math.floor(w.time / 1000)}:R>`).join('\n\n');
    return message.reply({ embeds: [infoEmbed(`**Total:** ${list.length}\n\n${lines}`, `📋 Warnings — ${target.user.tag}`)] });
  },
};
