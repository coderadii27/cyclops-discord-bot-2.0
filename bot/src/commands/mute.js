...
export default {
  ...
  async execute({ message, args }) {
    ...
    if (!target) return message.channel.send({ embeds: [errorEmbed('Mention a valid user. Usage: `?mute @user 10m reason`')] });
    if (!target.moderatable) return message.channel.send({ embeds: [errorEmbed('I cannot mute that user.')] });

    const durStr = args[1];
    const ms = parseDuration(durStr);
    if (!ms || ms <= 0) return message.channel.send({ embeds: [errorEmbed('Invalid duration. Examples: `30s`, `10m`, `2h`, `1d`')] });
    if (ms > MAX_TIMEOUT_MS) return message.channel.send({ embeds: [errorEmbed('Duration too long. Maximum is 28 days.')] });

    const reason = args.slice(2).join(' ') || 'No reason provided';
    await target.timeout(ms, `${reason} • by ${message.author.tag}`);

    await message.channel.send({ embeds: [successEmbed(`🔇 **${target.user.tag}** muted for **${formatDuration(ms)}**.\n**Reason:** ${reason}`, 'User Muted')] });
